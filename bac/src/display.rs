...
pub fn output(weather_data: weather::OpenMeteoResponse, air_quality_data: weather::AirQuality, local_time: DateTime<Local>, ip_info: location::IPInfo, config: config::Config) -> String {
    ...
    if config.use_color {
        if !config.use_imperial {
            data.push_str(&format!("{} {}°C\n", "Temperature:".bold().bright_blue(), weather_data.current.temperature));
            data.push_str(&format!("    {} {}°C\n", "- Feels Like:".bold().bright_blue(), weather_data.current.feels_like));
            data.push_str(&format!("    {} {}°C\n", "- Today's High:".bold().bright_blue(), weather_data.daily.temperature_max[0]));
            data.push_str(&format!("    {} {}°C\n", "- Today's Low:".bold().bright_blue(), weather_data.daily.temperature_min[0]));
            data.push_str(&format!("    {} {}\n", "- UV Index:".bold().bright_blue(), weather_data.daily.uv_index[0]));
            data.push_str(&format!("        {} {}\n", "- Should you bring sunscreen?".bold().bright_blue(), should_you_bring_sunscreen(weather_data.daily.uv_index[0])));
            data.push_str(&format!("{} {}\n", "Sky Condition:".bold().bright_blue(), cloudcover_to_description(weather_data.current.sky_condition_num)));
            data.push_str(&format!("    {} {}%\n", "- Cloud Cover:".bold().bright_blue(), weather_data.current.sky_condition_num));
            data.push_str(&format!("{} {}\n", "Wind:".bold().bright_blue(), kmph_to_description(weather_data.current.wind_speed)));
            data.push_str(&format!("    {} {} km/h\n", "- Speed:".bold().bright_blue(), weather_data.current.wind_speed));
            data.push_str(&format!("    {} {} ({}°)\n", "- Direction:".bold().bright_blue(), deg_to_compass(weather_data.current.wind_direction), weather_data.current.wind_direction));
            data.push_str(&format!("{} {}\n", "Humidity:".bold().bright_blue(), humidity_to_description(weather_data.current.humidity)));
            data.push_str(&format!("    {} {}%\n", "- Relative Humidity:".bold().bright_blue(), weather_data.current.humidity));
            data.push_str(&format!("    {} {}°C\n", "- Dew Point:".bold().bright_blue(), weather_data.hourly.dew_point[time_index]));
            data.push_str(&format!("{} {}%\n", "Chance of Precipitation:".bold().bright_blue(), weather_data.daily.precipitation_probability[0]));
            data.push_str(&format!("{} {}\n", "Visibility:".bold().bright_blue(), visibility_to_description(weather_data.hourly.visibility[time_index])));
            data.push_str(&format!("    {} {} km\n", "- Visibility Distance:".bold().bright_blue(), weather_data.hourly.visibility[time_index] / 1000.0));
            data.push_str(&format!("{} {}\n", "Air Quality:".bold().bright_blue(), aqi_to_description(air_quality_data.current.aqi)));
            data.push_str(&format!("    {} {}\n", "- US AQI:".bold().bright_blue(), air_quality_data.current.aqi));
            data.push_str(&format!("{} {}\n", "Today's sunrise:".bold().bright_blue(), sunrise));
            data.push_str(&format!("{} {}\n\n", "Today's sunset:".bold().bright_blue(), sunset));
        } else {
            data.push_str(&format!("{} {}°F\n", "Temperature:".bold().bright_blue(), c_to_f(weather_data.current.temperature).round()));
            data.push_str(&format!("    {} {}°F\n", "- Feels Like:".bold().bright_blue(), c_to_f(weather_data.current.feels_like).round()));
            data.push_str(&format!("    {} {}°F\n", "- Today's High:".bold().bright_blue(), c_to_f(weather_data.daily.temperature_max[0]).round()));
            data.push_str(&format!("    {} {}°F\n", "- Today's Low:".bold().bright_blue(), c_to_f(weather_data.daily.temperature_min[0]).round()));
            data.push_str(&format!("    {} {}\n", "- UV Index:".bold().bright_blue(), weather_data.daily.uv_index[0]));
            data.push_str(&format!("        {} {}\n", "- Should you bring sunscreen?".bold().bright_blue(), should_you_bring_sunscreen(weather_data.daily.uv_index[0])));
            data.push_str(&format!("{} {}\n", "Sky Condition:".bold().bright_blue(), cloudcover_to_description(weather_data.current.sky_condition_num)));
            data.push_str(&format!("    {} {}%\n", "- Cloud Cover:".bold().bright_blue(), weather_data.current.sky_condition_num));
            data.push_str(&format!("{} {}\n", "Wind:".bold().bright_blue(), kmph_to_description(weather_data.current.wind_speed)));
            data.push_str(&format!("    {} {} mph\n", "- Speed:".bold().bright_blue(), kmph_to_mph(weather_data.current.wind_speed).round()));
            data.push_str(&format!("    {} {} ({}°)\n", "- Direction:".bold().bright_blue(), deg_to_compass(weather_data.current.wind_direction), weather_data.current.wind_direction));
            data.push_str(&format!("{} {}\n", "Humidity:".bold().bright_blue(), humidity_to_description(weather_data.current.humidity)));
            data.push_str(&format!("    {} {}%\n", "- Relative Humidity:".bold().bright_blue(), weather_data.current.humidity));
            data.push_str(&format!("    {} {}°F\n", "- Dew Point:".bold().bright_blue(), c_to_f(weather_data.hourly.dew_point[time_index]).round()));
            data.push_str(&format!("{} {}%\n", "Chance of Precipitation:".bold().bright_blue(), weather_data.daily.precipitation_probability[0]));
            data.push_str(&format!("{} {}\n", "Visibility:".bold().bright_blue(), visibility_to_description(weather_data.hourly.visibility[time_index])));
            data.push_str(&format!("    {} {} miles\n", "- Visibility Distance:".bold().bright_blue(), kmph_to_mph(weather_data.hourly.visibility[time_index] / 1000.0).round()));
            data.push_str(&format!("{} {}\n", "Air Quality:".bold().bright_blue(), aqi_to_description(air_quality_data.current.aqi)));
            data.push_str(&format!("    {} {}\n", "- US AQI:".bold().bright_blue(), air_quality_data.current.aqi));
            data.push_str(&format!("{} {}\n", "Today's sunrise:".bold().bright_blue(), sunrise));
            data.push_str(&format!("{} {}\n\n", "Today's sunset:".bold().bright_blue(), sunset));
